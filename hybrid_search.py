...
from query_graph import GRAPHDB_URL, get_candidate_profile
...
def hybrid_search(job_description: str, candidate_name: str):
    ...
    # Truncate to avoid ContextWindowExceededError ---
    # 350 words is safely under the 512 token limit for most tokenizers
    safe_job_description = " ".join(job_description.split()[:250])
    logger.debug(
        f"Truncated job description to {len(safe_job_description.split())} words for embedding."
    )
    ...
    if hybrid_results["results"]["bindings"]:
        ...
        logger.warning(
            "Vectors matched, but Graph constraints failed (no relevant work history)."
        )
        return {
            "message": "No semantically relevant work history found for this candidate."
        }
